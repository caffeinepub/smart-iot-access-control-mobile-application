...
actor {
  ...
  // Todos specific code
  public shared ({ caller }) func createTodo(
    title : Text,
    description : Text,
    priority : Priority,
    category : Text,
      ...
      case (?existingTodo) {
        ...
        let updatedTodo : ToDo = {
          ...
          priority = switch (priority) {
            case (?p) { p };
            case (_) { existingTodo.priority };
          };
          category = switch (category) {
            case (?c) { c };
            case (_) { existingTodo.category };
          };
          dueDate;
          subtasks = switch (subtasks) {
            case (?s) { s };
            case (_) { existingTodo.subtasks };
          };
          timerSeconds;
          timeSpent = switch (timeSpent) {
            case (?t) { t };
            case (_) { existingTodo.timeSpent };
          };
        };

        todos.add(id, updatedTodo);

        // Update user stats if the todo was marked as completed
        switch (completed) {
          case (?true) {
            updateUserStatsOnTodoCompletion(caller);
          };
          case (_) {};
        };
      };
  ...
  public shared ({ caller }) func completeTodo(id : Nat) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only users can complete todos");
    };

    switch (todos.get(id)) {
      case (null) { Runtime.trap("TODO does not exist") };
      case (?todo) {
        if (todo.owner != caller and not AccessControl.isAdmin(accessControlState, caller)) {
          Runtime.trap("Unauthorized: You can only complete your own todos");
        };

        let updatedTodo = { todo with completed = true };
        todos.add(id, updatedTodo);
  ...
  public query ({ caller }) func getTodosByCategory(category : Text) : async [ToDo] {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only users can view todos");
    };

    let allTodos = if (AccessControl.isAdmin(accessControlState, caller)) {
      todos.values().toArray();
    } else {
      todos.values().toArray().filter(func(t) { t.owner == caller });
    };

    allTodos.filter(func(todo) { todo.category == category });
  };

  public shared ({ caller }) func startTimer(todoId : Nat) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only users can start timers");
    };

    let todo = getTodoByIdInternal(todoId);
    if (todo.owner != caller) {
      Runtime.trap("Unauthorized: You can only start timers for your own todos");
    };

    // Use timerSeconds as is - no state change needed since we assume frontend tracks time
  };

  public shared ({ caller }) func finishTimer(todoId : Nat, timeSpent : Nat) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only users can finish timers");
    };

    let todo = getTodoByIdInternal(todoId);
    if (todo.owner != caller) {
      Runtime.trap("Unauthorized: You can only finish timers for your own todos");
    };

    let updatedTodo = { todo with timeSpent };
    todos.add(todoId, updatedTodo);
  };

  func getTodoByIdInternal(id : Nat) : ToDo {
    switch (todos.get(id)) {
      case (null) { Runtime.trap("Todo does not exist") };
      case (?todo) { todo };
    };
  ...
  public query ({ caller }) func getTodoStats() : async {
    ...
    let allTodos = todos.values().toArray().filter(func(t) { t.owner == caller });
    let total = allTodos.size();
    let completed = allTodos.filter(func(t) { t.completed }).size();
    let urgent = allTodos.filter(func(t) { t.priority == #high }).size();
    let overdue = allTodos.filter(func(t) {
      switch (t.dueDate) {
        case (null) { false };
        case (?due) { due < Time.now() and not t.completed };
      }
    }).size();
    let pending = allTodos.filter(func(t) { not t.completed }).size();
  ...
  public query ({ caller }) func getMyStats() : async UserStats {
    ...
    switch (userStats.get(caller)) {
      case (null) { Runtime.trap("User stats do not exist") };
      case (?stats) { stats };
    };
  };

  func updateUserStatsOnTodoCompletion(caller : Principal) {
    let now = Time.now();
    let today = now / 86_400_000_000_000 : Int; // Daily streak granularity
    ...
    let newStreak = switch (currentStats.lastCompletedDate) {
      case (null) { 1 };
      case (?lastDate) {
        let daysSinceLast = today - lastDate;
        if (daysSinceLast == 1) { // 1 day
          currentStats.currentStreak + 1;
        } else { 1 };
      };
    };

    let newPoints = currentStats.points + 1; // Adjust points calculation if needed
    ...
    let newStats = {
      ...
      longestStreak = if (newStreak > currentStats.longestStreak) {
        newStreak;
      } else { currentStats.longestStreak };
  ...
  public shared ({ caller }) func createDefaultStats() : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can create default stats");
    };
