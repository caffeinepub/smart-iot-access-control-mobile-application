...
import {
  useDisableVacationMode,
  useEnableVacationMode,
  useGetVacationModeStatus,
...
export default function VacationModeToggle() {
  ...
  const handleToggle = (checked: boolean) => {
    if (checked) {
      enableMutation.mutate(undefined, {
        onSuccess: () =>
          toast.success("Vacation Mode activated — all access suspended"),
  ...
  return (
    <div
      className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${isActive ? "border-warning/40 bg-warning/5" : "border-border bg-card"}`}
    >
      <Palmtree
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          {isActive
            ? "Active — all automation rules suspended, full lockdown"
            : "Suspend all rules and activate full lockdown"}
        </p>
      </div>
      {isPending ? (
