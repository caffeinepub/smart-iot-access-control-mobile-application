...
import { Activity, Clock, MemoryStick, Thermometer } from "lucide-react";
    ...
    <Card className="border-accent/10">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant="outline"
            className={`text-xs ${statusBadgeClass(metrics.status)}`}
          >
      ...
      <CardContent className="space-y-3">
        <div className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={`font-mono font-semibold ${statusColor(metrics.status)}`}
            >
              {metrics.uptime}%
            </span>
          </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={`font-mono font-semibold ${metrics.temperature > 55 ? "text-destructive" : metrics.temperature > 45 ? "text-warning" : "text-success"}`}
            >
          ...
          <div className="w-full bg-muted/40 rounded-full h-1.5">
            <div
              ...
              style={{
                width: `${Math.min(100, (metrics.temperature / 80) * 100)}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={`font-mono font-semibold ${metrics.memoryUsage > 80 ? "text-destructive" : metrics.memoryUsage > 65 ? "text-warning" : "text-success"}`}
            >
              {metrics.memoryUsage}%
            </span>
          </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={`font-mono font-semibold ${metrics.cpuLoad > 85 ? "text-destructive" : metrics.cpuLoad > 70 ? "text-warning" : "text-success"}`}
            >
              {metrics.cpuLoad}%
            </span>
          </div>
