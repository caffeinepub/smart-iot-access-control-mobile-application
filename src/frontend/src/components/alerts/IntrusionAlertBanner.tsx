...
export default function IntrusionAlertBanner({
  recentFailedEvent,
}: IntrusionAlertBannerProps) {
  const [dismissed, setDismissed] = useState(false);

  if (!recentFailedEvent || dismissed) return null;

  const time = new Date(
    Number(recentFailedEvent.timestamp) / 1_000_000,
  ).toLocaleTimeString();
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground truncate">
            Failed access by{" "}
            {recentFailedEvent.userEmail ||
              recentFailedEvent.rfidUid ||
              "Unknown"}{" "}
            at {time}
          </p>
        </div>
