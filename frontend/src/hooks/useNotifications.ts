...
export type NotificationCategory = 'Access Events' | 'System Alerts' | 'Automation';

export interface AppNotification {
  id: string;
  type: 'access_granted' | 'access_denied' | 'intrusion' | 'device_offline' | 'system';
...
function assignCategory(type: AppNotification['type'], message: string): NotificationCategory {
  if (type === 'access_granted' || type === 'access_denied') return 'Access Events';
  if (type === 'intrusion' || type === 'device_offline') return 'System Alerts';
  const lower = message.toLowerCase();
  if (lower.includes('rule') || lower.includes('automation') || lower.includes('smart')) return 'Automation';
  if (lower.includes('intrusion') || lower.includes('tamper') || lower.includes('device') || lower.includes('offline')) return 'System Alerts';
  return 'System Alerts';
}

function eventsToNotifications(events: AccessEvent[], logs: LogEvent[]): AppNotification[] {
  const notifs: AppNotification[] = [];

  events.slice(0, 20).forEach((event) => {
    const ts = Number(event.timestamp) / 1_000_000;
    const type: AppNotification['type'] = event.success ? 'access_granted' : 'access_denied';
    notifs.push({
...
export function useNotifications(events: AccessEvent[] = [], logs: LogEvent[] = []) {
