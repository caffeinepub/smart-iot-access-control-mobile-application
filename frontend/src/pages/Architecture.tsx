...
import { Network, Play, Pause, Layers } from 'lucide-react';
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">System topology and data flow visualization</p>
        </div>
        <Button
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>ESP32 → ICP Backend → React Frontend</CardDescription>
            </div>
            <Badge variant="outline" className={isLive ? 'border-success/40 text-success bg-success/10 animate-pulse' : ''}>
              {isLive ? '● Live' : '○ Static'}
            </Badge>
          </div>
        </CardHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-accent/20">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-accent">ESP32 Device</CardTitle>
          </CardHeader>
        ...
        <Card className="border-accent/20">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-accent">ICP Backend</CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-muted-foreground space-y-1">
        ...
        <Card className="border-accent/20">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-accent">React Frontend</CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-muted-foreground space-y-1">
      ...
      <Card className="border-accent/20">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            {[
              { label: 'Sensor Data', dir: 'ESP32 → Backend', color: 'text-accent' },
              { label: 'Commands', dir: 'Backend → ESP32', color: 'text-warning' },
              { label: 'Query Results', dir: 'Backend → Frontend', color: 'text-success' },
              { label: 'Update Calls', dir: 'Frontend → Backend', color: 'text-destructive' },
            ].map((item) => (
              <div key={item.label} className="flex flex-col gap-1 p-2 rounded-lg bg-muted/30 border border-border">
