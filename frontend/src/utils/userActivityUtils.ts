...
export function computeUserActivityMetrics(events: AccessEvent[]): Map<string, UserActivityMetrics> {
  const map = new Map<string, { total: number; success: number; lastTs: bigint }>();

  events.forEach((event) => {
    const key = event.userEmail || event.rfidUid || 'Unknown';
    const existing = map.get(key) ?? { total: 0, success: 0, lastTs: BigInt(0) };
    existing.total++;
