...
export function calculateAverageTemperature(devices: DeviceHealthData[]): number {
  if (devices.length === 0) return 0;
  return Math.round(devices.reduce((sum, d) => sum + d.temperature, 0) / devices.length);
}

export function calculateAverageMemoryUsage(devices: DeviceHealthData[]): number {
  if (devices.length === 0) return 0;
  return Math.round(devices.reduce((sum, d) => sum + d.memoryUsage, 0) / devices.length);
}

export function countWarningDevices(devices: DeviceHealthData[]): number {
  return devices.filter((d) => (d.temperature > 70 || d.memoryUsage > 80) && d.temperature <= 85 && d.memoryUsage <= 95).length;
}
