  ...
  return (
    <Card className="border-accent/20">
      <CardHeader>
        ...
        <CardDescription>Recent access events ({events.length} total)</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <div className="space-y-3">
            {sortedEvents.length === 0 ? (
              ...
              sortedEvents.map((event, index) => (
                <div
                  key={index}
                  className="flex items-start gap-4 p-4 rounded-lg border border-accent/10 bg-card hover:bg-accent/5 transition-colors"
                >
                  <div className="mt-1">
                    {event.success ? (
