  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden">
      <Table>
        ...
        <TableBody>
          {rows.map((row, idx) => {
            const rate = row.total > 0 ? Math.round((row.successCount / row.total) * 100) : 0;
            return (
              <TableRow key={idx} className="hover:bg-muted/20">
                <TableCell className="font-mono text-xs text-muted-foreground">{row.period}</TableCell>
                <TableCell className="text-sm max-w-[180px] truncate">{row.userEmail}</TableCell>
                ...
                <TableCell className="text-center font-mono text-sm">{row.total}</TableCell>
                <TableCell className="text-center">
                  <span className={`text-xs font-semibold ${rate >= 80 ? 'text-success' : rate >= 50 ? 'text-warning' : 'text-destructive'}`}>
                    {rate}%
                  </span>
                </TableCell>
              </TableRow>
