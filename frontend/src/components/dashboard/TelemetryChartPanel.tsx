...
export default function TelemetryChartPanel() {
  ...
  const chartData = useMemo((): TelemetryPoint[] => {
    ...
    for (let i = 0; i < maxPoints; i++) {
      let tempSum = 0, cpuSum = 0, memSum = 0, count = 0;
    ...
    <Card className="border-accent/20 shadow-lg shadow-accent/5">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Device Telemetry
          <span className="ml-auto text-xs text-muted-foreground font-normal">Updates every 10s</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {chartData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
              ...
              <Line type="monotone" dataKey="temperature" stroke="oklch(var(--destructive))" strokeWidth={2} dot={false} name="Temperature" />
              <Line type="monotone" dataKey="cpu" stroke="oklch(var(--accent))" strokeWidth={2} dot={false} name="CPU" />
              <Line type="monotone" dataKey="memory" stroke="oklch(var(--warning))" strokeWidth={2} dot={false} name="Memory" />
            </LineChart>
          </ResponsiveContainer>
        )}
      </CardContent>
