...
import { ShieldCheck, Thermometer, MemoryStick, AlertTriangle } from 'lucide-react';
...
function HealthGauge({ score }: { score: number }) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center p-4 rounded-xl border ${bgColor}`}>
    ...
    <Card className="border-accent/20 shadow-lg shadow-accent/5">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <span className="ml-auto text-xs text-muted-foreground font-normal">Refreshes every 30s</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/30 border border-border">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold font-mono">{stats.avgTemp}°C</p>
                </div>
              </div>
              <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/30 border border-border">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 p-3 rounded-lg bg-warning/10 border border-warning/30">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold font-mono text-warning">{stats.warnings}</p>
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/30">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold font-mono text-destructive">{stats.criticals}</p>
                  <p className="text-xs text-muted-foreground">devices</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 rounded-lg bg-muted/30 border border-border">
              ...
              {currentMetrics.map((d) => (
                <div key={d.deviceId} className="flex items-center justify-between text-xs mb-1">
                  <span className="truncate text-muted-foreground max-w-[80px]">{d.deviceId.slice(0, 8)}</span>
                  <span className={`font-mono font-semibold ${
                    d.temperature > 85 || d.memoryUsage > 95 ? 'text-destructive' :
                    d.temperature > 70 || d.memoryUsage > 80 ? 'text-warning' : 'text-success'
                  }`}>
                    {d.temperature}°C
                  </span>
                </div>
