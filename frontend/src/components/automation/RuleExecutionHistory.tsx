...
import { History, CheckCircle, XCircle, ClipboardList } from 'lucide-react';
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-3">
            ...
            <p className="text-xs opacity-70">Rule triggers will appear here when automation rules fire</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-sm font-semibold">{log.ruleName}</span>
                      <Badge
                        variant={log.outcome ? 'default' : 'destructive'}
                        className={`text-xs ${log.outcome ? 'bg-success/20 text-success border-success/30' : ''}`}
                      >
                        {log.outcome ? 'Success' : 'Failed'}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                      <span>Trigger: <span className="text-foreground">{log.triggerCondition}</span></span>
                      <span>•</span>
                      <span>Action: <span className="text-foreground">{log.actionTaken}</span></span>
                    </div>
