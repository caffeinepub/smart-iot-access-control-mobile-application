...
export default function AccessWindowModal({ open, onClose, uid, userName }: Props) {
  ...
  useEffect(() => {
    ...
    const newSchedule: DaySchedule[] = DAYS.map(() => ({ enabled: false, startHour: 9, endHour: 17 }));
  ...
  const updateDay = (i: number, field: keyof DaySchedule, value: boolean | number) => {
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="bg-card border-border max-w-lg">
          ...
          <div className="space-y-2 py-2 max-h-[60vh] overflow-y-auto">
            {DAYS.map((day, i) => (
              <div key={day} className={`flex items-center gap-3 p-2.5 rounded-lg border transition-colors ${
                schedule[i].enabled ? 'border-primary/40 bg-primary/5' : 'border-border bg-background/30'
              }`}>
                <Checkbox
                  id={`day-${i}`}
                ...
                />
                <Label htmlFor={`day-${i}`} className="w-24 text-sm cursor-pointer">{day}</Label>
                {schedule[i].enabled && (
                  <div className="flex items-center gap-2 flex-1">
                    <select
                      value={schedule[i].startHour}
                      onChange={e => updateDay(i, 'startHour', Number(e.target.value))}
                    ...
                    <select
                      ...
                      onChange={e => updateDay(i, 'endHour', Number(e.target.value))}
                      className="bg-background border border-border rounded px-2 py-1 text-xs text-foreground"
                    >
                      {Array.from({ length: 24 }, (_, h) => (
                        <option key={h} value={h}>{h.toString().padStart(2, '0')}:00</option>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={mutation.isPending || isLoading} className="flex items-center gap-1.5">
            {mutation.isPending ? (
