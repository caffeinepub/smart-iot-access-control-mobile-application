...
import { UserX, UserCheck, Trash2, Clock } from 'lucide-react';
...
export default function UserList() {
  ...
  const [accessWindowUser, setAccessWindowUser] = useState<{ uid: string; name: string } | null>(null);
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border">
              ...
              <TableHead className="text-muted-foreground text-xs">Activity</TableHead>
              {isAdmin && <TableHead className="text-muted-foreground text-xs">Actions</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
            {users.map(user => {
              const activity = activityMap.get(user.rfidUid) || activityMap.get(user.email);
              const successRate = activity ? Math.round(activity.successRate * 100) : null;
              const successColor = successRate === null ? '' : successRate >= 90 ? 'text-green-400' : successRate >= 70 ? 'text-yellow-400' : 'text-red-400';

              return (
                <TableRow key={user.rfidUid} className="border-border hover:bg-muted/20">
                  <TableCell className="text-sm font-medium text-foreground">{user.name}</TableCell>
                  <TableCell className="text-sm text-muted-foreground">{user.email}</TableCell>
                  <TableCell>
                    <Badge variant={user.role === 'admin' ? 'default' : user.role === 'guest' ? 'secondary' : 'outline'} className="text-xs">
                      {user.role}
                    </Badge>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {activity ? (
                      <div className="text-xs space-y-0.5">
                        <div className="text-muted-foreground">{activity.accessCount} accesses</div>
                        {successRate !== null && (
                          <div className={`font-mono ${successColor}`}>{successRate}% success</div>
                        )}
                      </div>
                    ) : (
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => setAccessWindowUser({ uid: user.rfidUid, name: user.name })}
