...
const CATEGORIES: Array<'All' | NotificationCategory> = ['All', 'Access Events', 'System Alerts', 'Automation'];

function getIcon(type: AppNotification['type']) {
  switch (type) {
    case 'access_granted': return <CheckCircle className="w-4 h-4 text-success" />;
    case 'access_denied': return <XCircle className="w-4 h-4 text-destructive" />;
    case 'intrusion': return <AlertTriangle className="w-4 h-4 text-warning" />;
...
function getCategoryColor(category: NotificationCategory): string {
  switch (category) {
    case 'Access Events': return 'bg-accent/15 text-accent border-accent/30';
    case 'System Alerts': return 'bg-warning/15 text-warning border-warning/30';
    case 'Automation': return 'bg-chart-5/15 text-chart-5 border-chart-5/30';
  }
}
            ...
            >
              ...
              {count > 0 && (
                <span className={`text-xs rounded-full px-1 min-w-[16px] text-center ${
                  activeFilter === cat ? 'bg-accent-foreground/20 text-accent-foreground' : 'bg-accent/20 text-accent'
                }`}>
                  {count}
                </span>
              )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium shrink-0 ${getCategoryColor(notif.category)}`}>
                      {notif.category}
                    </span>
                  </div>
