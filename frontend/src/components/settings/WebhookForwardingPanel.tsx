...
import { Webhook, Send, CheckCircle, Trash2 } from 'lucide-react';
...
function loadConfig(): WebhookConfig {
  ...
  return { url: '', alertTypes: { intrusion: true, lockdown: false, firmware: false } };
}
...
export default function WebhookForwardingPanel() {
  ...
  const simulateWebhook = useCallback((eventType: string, payload: object) => {
    ...
    if (recent.length > 0) {
      ...
      if (!sessionStorage.getItem(key)) {
        ...
        simulateWebhook('intrusion', {
          event: 'ACCESS_DENIED',
          timestamp: new Date(Number(latest.timestamp) / 1_000_000).toISOString(),
          deviceId: 'default',
  ...
  const handleAlertTypeChange = (type: keyof WebhookConfig['alertTypes'], checked: boolean) => {
    const updated = { ...config, alertTypes: { ...config.alertTypes, [type]: checked } };
    setConfig(updated);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(updated));
  };
    ...
    <Card className="bg-card border-border">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label className="text-sm text-muted-foreground">Webhook URL (mock)</Label>
          ...
          <div className="flex flex-wrap gap-4">
            {(['intrusion', 'lockdown', 'firmware'] as const).map(type => (
              <div key={type} className="flex items-center gap-2">
                ...
                />
                <Label htmlFor={`webhook-${type}`} className="text-sm capitalize cursor-pointer">
                  {type}
                </Label>
              </div>
            ))}
        ...
        <Button size="sm" variant="outline" onClick={handleTestWebhook} disabled={!config.url} className="flex items-center gap-2">
        ...
        {logs.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-sm text-muted-foreground">Webhook Log (last 10)</Label>
            ...
            <ScrollArea className="h-48 rounded border border-border">
              <div className="p-2 space-y-2">
                {logs.map(log => (
                  <div key={log.id} className="bg-background/50 rounded p-2 text-xs space-y-1">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="font-mono text-muted-foreground truncate">{log.endpoint}</div>
